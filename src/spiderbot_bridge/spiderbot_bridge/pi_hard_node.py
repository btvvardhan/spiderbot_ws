...
- Subscribes to 'cmd' from main computer
...
class PiBridgeNode(Node):
    def __init__(self):
        ...
        # Subscribe to ASCII commands from main computer (keyboard teleop)
        self.cmd_sub = self.create_subscription(
            String,
            'cmd',              # topic name from your teleop node
            self.cmd_callback,
            10
        )
        ...
        # State tracking for serial writes
        self.last_sent_line = ""
        self.send_count = 0
        self.skip_count = 0
        ...
        self.get_logger().info('   📥 Subscribing: /cmd, /scan')
        self.get_logger().info('   📤 Publishing: /imu, /mag, /scan_republished')

    # NEW: forward 'cmd' topic to Arduino over serial
    def cmd_callback(self, msg: String):
        """
        Receive command characters from main computer (topic 'cmd')
        and send them to Arduino over serial.
        """
        cmd_line = msg.data

        # Ensure exactly one newline at the end (your teleop already adds it,
        # but this makes the node robust if something else publishes to 'cmd')
        if not cmd_line.endswith('\n'):
            cmd_line = cmd_line + '\n'

        # Extract the first non-newline character to know what command this is
        cmd_char = cmd_line.strip()[:1] if cmd_line.strip() else ""

        # Skip duplicates for gait commands, but NOT for speed commands (+/-)
        if cmd_char in ['w', 's', 'a', 'd', 'q', 'e', 'x'] and cmd_line == self.last_sent_line:
            self.skip_count += 1
            return

        try:
            self.ser.write(cmd_line.encode('ascii', errors='ignore'))
            self.ser.flush()
            self.last_sent_line = cmd_line
            self.send_count += 1

            if self.send_count % 50 == 0:
                self.get_logger().info(
                    f'📤 CMD to Arduino: #{self.send_count} '
                    f'(skipped {self.skip_count} dups) last="{cmd_char}"'
                )
                self.skip_count = 0
